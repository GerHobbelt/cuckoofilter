...
Let ``$x_i[p,q)$'' denote the bits in $x$ at locations $p, \dots, q-1$, numbering from zero, i.e. $\lfloor (x_i - \lfloor x_i/2^q \rfloor 2^q)/2^p \rfloor$
Denote $m[i][x_i[k-w,k)]$ by $t_i$.
We now consider two separate cases, depending on the value of $k - w - \lg |t_i|$.
From this, we will construct a multi-set $z_i$ of slots in $t_i$ and a single value $y_i$.
$z_i$ will have size $2^{\max(0, w + \lg |t_i| - k)}$

\begin{itemize}
\item If $k - w - \lg |t_i| \geq 0$, then $z_i = \{x_i[k - w - \lg |t_i|, k - w) \}$ and $y_i = y 2^{k - w - \lg |t_i|} + x_i[0, k - w - \lg |t_i|)$.
\item If $k - w - \lg |t_i| < 0$, then $z_i = \{x_i[k - w - \lg |t_i|, k - w) 2^{w + \lg |t_i| - k} + j : 0 \leq j < 2^{w + \lg |t_i| - k}\}$ and $y_i = y$.
...
In either case $y_i \leq 2^{v + \max(0, k - w - \lg |t_i|)}$.

The values associated with the key $x$ are 
