...
Note that $l \geq \lg n - (2w + 1) \lg d$, since there must be at least $n$ slots.
%Each sub-table is either ``small'' or ``large''.
...
$|y_0|$ in the first case is the right size since $|x_2| = k - w - l$ and $|y| = v$

UHOH: in the second case, $y_0$ has size $v$, when it should have size $v + (k - w - l)$, and the second part is less than zero.
So it is too big.
Change the value size to $k + v - w - l + s + O(1)$.
...
The total space usage is at most $d 2^w$ pointers, plus $d 2^w$ bits (one for each sub-table to indicate if it is ``small''), plus $\alpha n (k + v - w + s - l)$ bits, where $l$
