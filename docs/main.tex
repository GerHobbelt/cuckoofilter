...
In this section, we'll show a practical map data structure that can store a map from $n \geq 128$ random keys that are $k > \lg n$ bits in length to $v$-bit values (excluding the value $0$) in low space.
The basic structure follows that of dynamic space efficient cuckoo tables~\cite{maier2019dynamic}.
...
This construction can be repeated with fewer magic constants:

Let $s > 0$ be an integral number of shingle bits.
...
We map from $n \geq d 2^{s+w}$ random keys that are $k > \lg n$ bits in length to $v$-bit values (excluding the value $0$).
Each sub-table is either ``small'' or ``large''.
Small sub-tables have $2^{\lceil \lg (\alpha n / d) \rceil - w - 1}$ slots, while large sub-tables have twice that.
There must be at least one ``small'' sub-table.
In a sub-table of length $l$, the values are of length $k + v - w + s - \lg l$.
...
The pair $(x,y)$ is said to be present in the dictionary if there exist $0 < i < d$ and $0 < j < 2^s$ such that in directory $i$, in sub-table $x^i_0$, in slot $x^i_1 + j) \bmod 2^{l^i}$, the value if $j + 2^s y^i_0$.
