...
Let $x_0 = x$ and $x_i = (x + h_i(y)) \bmod 2^k$ for $0 < i < d$.
...
Let $z_i = \{x_i[k - w - \lg |t_i|, k - w) 2^{\max(0,w + \lg |t_i| - k)} \}$ be a slot in $t_i$. % and let $y_i = y + x_i[0, \max(0,k - w - \lg |t_i|)) 2^v$.
%The bitlength of $y_i$ is $v + \max(0, k - w - \lg |t_i|)$.

The values associated with $x_i$ are derived from $r_{i,j} = t_i[(z_i  + j) \bmod |t_i|]$ for $0 \leq j < 2^s$.
When $k \geq w + \lg |t_i|$, the values associated with $x_i$ are derived from the values in $r_{i,j}$ as follows:

$$y_i = \left\{r_{i,j}[s, s+v) : r_{i,j}[0,s) = j \land r_{i,j}[s+v,s + v + \max(0,k - w - \lg |t_i|)) = x_i[0, \max(0,k - w - \lg |t_i|))\right\}.$$

%% When $k < w + \lg |t_i|$,

%% $$y_i = \{r_{i,j}[s, s+v) : r_{i,j}[0,s) = j \}.$$

The values associates with $x$ are $\cup_i y_i$, of which there are at most $d2^s$.
...
The total space usage is at most $d 2^w$ pointers to the sub-tables, plus $d 2^w$ bits (one for each sub-table to indicate if it is ``small''), plus $\alpha n (s + v + \max(0, k - w - l))$ bits, where $l$ is the size of the small sub-tables.
